...
import { normalizeExpense, getLocalizedValue } from '../utils/foundationHelpers';
import {
  FoundationConfig,
  Member,
  Designation,
...
function extractMissingColumn(errorMessage: string): string | null {
  ...
  // PostgREST 12+ format: Could not find the 'XYZ' column of 'TABLE' in the schema cache
  const pgrstMatch = errorMessage.match(/['"]([a-zA-Z0-9_]+)['"]\s+column/i);
  if (pgrstMatch && pgrstMatch[1]) return pgrstMatch[1];
  // Postgres format: column "XYZ" of relation "TABLE" does not exist
  const pgMatch = errorMessage.match(/column\s+['"]?([a-zA-Z0-9_]+)['"]?/i);
...
export const supabaseService = {
  ...
  async saveMemberDetailed(m: Member): Promise<OperationResult<Member>> {
    ...
    try {
      ...
      let payload: Record<string, any> = {
        ...
        imo: m.imo || m.socialLinks?.imo || null,
      ...
      return (data || []).map((row) => ({
        ...
        fullDescription:
          typeof row.description === 'object' && row.description !== null
            ? row.description
            : { bn: row.description || '', en: row.description || '', ar: row.description || '' },
        purpose: typeof row.purpose === 'object' && row.purpose !== null ? row.purpose : undefined,
  ...
  async saveActivityDetailed(a: Activity): Promise<OperationResult<Activity>> {
    ...
    try {
      ...
      const payload: Record<string, any> = {
        ...
        slug: a.slug || a.id,
        title: a.title,
        summary: a.summary || a.shortSummary || { bn: '', en: '', ar: '' },
      ...
      while (error && (error.message?.includes('column') || error.code === 'PGRST204' || (error as any).code === '42703')) {
        ...
        if (errMsg.includes('gallery_images') && 'gallery_images' in payload) {
        ...
        if (errMsg.includes('slug') && 'slug' in payload) {
          delete payload.slug;
          stripped = true;
        }
        if (errMsg.includes('summary') && 'summary' in payload) {
        ...
        const retry = await supabase.from('activities').upsert(payload, { onConflict: 'id' });
        error = retry.error;
      }
      ...
      const verifiedActivity: Activity = {
        ...
        shortSummary: typeof verified.summary === 'object' && verified.summary !== null
          ? verified.summary
          : {
              bn: verified.summary || '',
              en: verified.summary || '',
    ...
    try {
      ...
      return (data || []).map((row) =>
        normalizeExpense({
          ...
          title: getLocalizedValue(row.title || row.purpose, 'bn') || 'সহায়তা ব্যয়',
          category: row.category || 'other',
          customCategory: row.custom_category ? getLocalizedValue(row.custom_category, 'bn') : undefined,
          description: row.description ? getLocalizedValue(row.description, 'bn') : undefined,
          recipient: row.recipient ? getLocalizedValue(row.recipient, 'bn') : undefined,
          isRecipientPublic: Boolean(row.is_recipient_public),
          location: row.location ? getLocalizedValue(row.location, 'bn') : undefined,
  ...
  async saveExpenseDetailed(e: ExpenseRecord): Promise<OperationResult<ExpenseRecord>> {
    ...
    try {
      ...
      while (error && (error.message?.includes('column') || error.code === 'PGRST204' || (error as any).code === '42703')) {
        ...
        if (errMsg.includes('recipient') && 'recipient' in payload) {
      ...
      const verifiedExpense: ExpenseRecord = normalizeExpense({
        ...
        title: getLocalizedValue(verified.title || verified.purpose, 'bn') || 'সহায়তা ব্যয়',
        category: verified.category || 'other',
        customCategory: verified.custom_category ? getLocalizedValue(verified.custom_category, 'bn') : undefined,
        description: verified.description ? getLocalizedValue(verified.description, 'bn') : undefined,
        recipient: verified.recipient ? getLocalizedValue(verified.recipient, 'bn') : undefined,
        isRecipientPublic: Boolean(verified.is_recipient_public),
        location: verified.location ? getLocalizedValue(verified.location, 'bn') : undefined,
  ...
  subscribeToChanges(callback: (payload: { table: string; eventType: string }) => void): () => void {
        ...
        channel = supabase
          ...
          .subscribe((status) => {
            if (status === 'TIMED_OUT' || status === 'CHANNEL_ERROR') {
              console.warn(`Supabase Realtime channel status: ${status}, retrying in 5s...`);
              if (!isDisposed) {
    ...
    // Auto-reconnect when device comes online
    const handleOnline = () => {
      setupChannel();
    };
