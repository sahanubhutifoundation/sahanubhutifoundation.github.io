...
export const AboutPage: React.FC = () => {
  ...
  return (
    <div className="space-y-12 sm:space-y-16 pb-16">
      {/* Page Hero */}
      <PageHero
        title={config.aboutHeroTitle ? tMulti(config.aboutHeroTitle) : t('navAbout')}
        subtitle={
          config.aboutHeroSubtitle
            ? tMulti(config.aboutHeroSubtitle)
            : language === 'bn'
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12 sm:space-y-16">
            ...
            <div className="flex items-center gap-3 mb-6 pb-4 border-b border-[#EBE8E0]">
              ...
              <div>
                <h2 className="text-lg sm:text-xl font-bold text-[#2D3630]">
                  {config.aboutSpeechTitle
                    ? tMulti(config.aboutSpeechTitle)
                    : language === 'bn'
                      ? 'ফাউন্ডেশনের উদ্বোধনী বার্তা'
                      : 'Inaugural Address of the Foundation'}
                </h2>
                <p className="text-xs text-[#7A877E]">
                  {config.aboutSpeechSubtitle
                    ? tMulti(config.aboutSpeechSubtitle)
                    : language === 'bn'
        ...
        {config.aboutShowBeginningSection !== false && (
          <section className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl sm:text-2xl font-bold text-[#2D3630]">
                {config.aboutStartTitle ? tMulti(config.aboutStartTitle) : t('aboutStartTitle')}
              </h2>
            </div>
            ...
            <div className="p-6 rounded-xl bg-[#F7F5F0] border border-[#EBE8E0] space-y-3 text-[#5C665F] text-sm sm:text-base leading-relaxed">
              ...
              {((config.aboutShowLocationCard !== false && config.locationDisplayMode !== 'hidden') ||
                (config.aboutShowDateCard !== false && config.showFoundingDate !== false && Boolean(config.foundedDate))) && (
                <div
                  className={`grid gap-3 pt-2 text-xs font-medium text-[#5C665F] ${
                    config.aboutShowLocationCard !== false &&
                    config.locationDisplayMode !== 'hidden' &&
                    config.aboutShowDateCard !== false &&
                    config.showFoundingDate !== false &&
                    Boolean(config.foundedDate)
        ...
        {config.aboutShowPurposeSection !== false && (
          <section className="space-y-4">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-lg bg-[#E8EFEA] text-[#2D5A41] border border-[#2D5A41]/20 flex items-center justify-center font-bold text-sm">
              ...
              <h2 className="text-xl sm:text-2xl font-bold text-[#2D3630]">
                {config.aboutPurposeTitle ? tMulti(config.aboutPurposeTitle) : t('aboutPurposeTitle')}
              </h2>
            </div>
            <div className="h-[2px] w-12 bg-[#2D5A41] rounded-full" />
            <div className="p-6 rounded-xl bg-white border border-[#EBE8E0] space-y-4 text-[#5C665F] text-sm sm:text-base leading-relaxed shadow-2xs">
              ...
              <div className="mt-4 p-5 rounded-xl bg-[#E8EFEA] border border-[#2D5A41]/20 space-y-2">
                <div className="flex items-center gap-2 text-[#1E3E2D] font-bold text-base">
                  ...
                  <h3>{config.aboutBaytulMalTitle ? tMulti(config.aboutBaytulMalTitle) : t('aboutBaytulMalTitle')}</h3>
                </div>
        ...
        {config.aboutShowValuesSection !== false && (
          <section className="space-y-4">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-lg bg-[#E8EFEA] text-[#2D5A41] border border-[#2D5A41]/20 flex items-center justify-center font-bold text-sm">
              ...
              <h2 className="text-xl sm:text-2xl font-bold text-[#2D3630]">
                {config.aboutValuesTitle ? tMulti(config.aboutValuesTitle) : t('aboutValuesTitle')}
              </h2>
            </div>
            <div className="h-[2px] w-12 bg-[#2D5A41] rounded-full" />
            
            {/* If structured cards are present and enabled */}
            {config.aboutValuesCards && config.aboutValuesCards.filter(c => c.isEnabled !== false).length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {config.aboutValuesCards
                  .filter(c => c.isEnabled !== false)
                  .map((card) => (
                    <div
                      key={card.id}
                      className="p-5 rounded-xl bg-white border border-[#EBE8E0] shadow-2xs space-y-2 hover:border-[#2D5A41]/40 transition-colors"
                    >
                      <div className="flex items-center gap-2.5">
                        <div className="w-7 h-7 rounded-lg bg-[#E8EFEA] text-[#2D5A41] flex items-center justify-center shrink-0">
                          <CheckCircle2 className="w-4 h-4" />
                        </div>
                        <h3 className="font-bold text-[#2D3630] text-base">
                          {tMulti(card.title)}
                        </h3>
                      </div>
                      {card.description && (
                        <p className="text-xs sm:text-sm text-[#5C665F] leading-relaxed pl-9">
                          {tMulti(card.description)}
                        </p>
                      )}
                    </div>
                  ))}
        ...
        {config.aboutShowVisionSection !== false && (
          <section className="space-y-4">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-lg bg-[#E8EFEA] text-[#2D5A41] border border-[#2D5A41]/20 flex items-center justify-center font-bold text-sm">
              ...
              <h2 className="text-xl sm:text-2xl font-bold text-[#2D3630]">
                {config.aboutFutureTitle ? tMulti(config.aboutFutureTitle) : t('aboutFutureTitle')}
              </h2>
            </div>
            <div className="h-[2px] w-12 bg-[#2D5A41] rounded-full" />
            <div className="p-6 rounded-xl bg-[#18231B] text-[#D3DDD5] border border-[#28382C] space-y-4 shadow-sm">
              ...
              {/* If structured future cards are present */}
              {config.aboutFutureCards && config.aboutFutureCards.filter(c => c.isEnabled !== false).length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                  {config.aboutFutureCards
                    .filter(c => c.isEnabled !== false)
                    .map((card) => (
                      <div
                        key={card.id}
                        className="p-3.5 bg-[#202C23] rounded-xl border border-[#28382C] space-y-1.5"
                      >
                        <div className="flex items-center gap-2">
                          <span className="w-2 h-2 rounded-full bg-[#65B78A] shrink-0" />
                          <h4 className="font-bold text-sm text-[#EAEFEA]">
                            {tMulti(card.title)}
                          </h4>
                        </div>
                        {card.description && (
                          <p className="text-xs text-[#9EAEA2] pl-4 leading-relaxed">
                            {tMulti(card.description)}
                          </p>
                        )}
                      </div>
