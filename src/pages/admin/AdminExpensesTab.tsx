...
export const AdminExpensesTab: React.FC<AdminExpensesTabProps> = ({
  ...
  const sheetReceived = fundPreview?.amountReceived || 0;
  const sheetSummaryCost = fundPreview?.sheetSummaryCost || 0;
  const sheetSummaryBalance = fundPreview?.sheetSummaryBalance || (sheetReceived - sheetSummaryCost);
  ...
  // Auto-scroll to form when opened or edited
  useEffect(() => {
    if (editingExpense) {
      setTimeout(() => {
  ...
  }, [editingExpense !== null, editingExpense?.id]);

  // Filtered expenses with guaranteed safe string normalization
  const filteredExpenses = expenses
    .map((e) => normalizeExpense(e))
    .filter((item) => {
      const titleStr = getLocalizedValue(item.title, 'bn');
      const recipientStr = item.recipient ? getLocalizedValue(item.recipient, 'bn') : '';
      const locStr = item.location ? getLocalizedValue(item.location, 'bn') : '';
                ...
                filteredExpenses.map((exp) => (
                  <tr key={exp.id} className="hover:bg-[#FDFCF9] transition-colors">
                    ...
                    <td className="py-3 px-4">
                      <div className="font-bold text-[#2D3630]">
                        {getLocalizedValue(exp.title, 'bn')}
                      </div>
                      {exp.description && (
                    ...
                    <td className="py-3 px-4 text-[#5C665F]">
                      <div className="flex items-center gap-1 font-medium">
                        {exp.recipient ? (
                          <>
                            <span>{getLocalizedValue(exp.recipient, 'bn')}</span>
                      ...
                      {exp.location && (
                        <span className="text-[10px] text-[#7A877E] flex items-center gap-0.5 mt-0.5">
                          ...
                          {getLocalizedValue(exp.location, 'bn')}
                        </span>
                      )}
                    </td>
