...
import { ActivityRailCard } from '../components/ActivityRailCard';
...
export const HomePage: React.FC = () => {
  ...
  const activitiesRailRef = React.useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    const refreshData = () => {
      ...
      // Activities canonical pipeline:
      // published activities -> ordering/featured selection -> homepage preview limit -> slice before render
      const allActivities = storageService.getActivities();
      const published = allActivities.filter((a) => a.isPublished !== false);
      const orderingMode = currentConfig.homeActivitiesOrdering || 'latest';
      const featuredIds = currentConfig.homeFeaturedActivityIds || [];

      let orderedActivities: Activity[] = [];
      if (orderingMode === 'manual' || (featuredIds.length > 0 && orderingMode === 'featured')) {
        const featured = featuredIds
          .map((id) => published.find((a) => a.id === id))
          .filter((a): a is Activity => !!a);

        if (orderingMode === 'featured') {
          const remaining = published
            .filter((a) => !featuredIds.includes(a.id))
            .sort((a, b) => (b.date || '').localeCompare(a.date || ''));
          orderedActivities = [...featured, ...remaining];
        } else {
          // Exact manual selection; if none selected, fallback to latest published
          orderedActivities = featured.length > 0 ? featured : [...published].sort((a, b) => (b.date || '').localeCompare(a.date || ''));
        }
      } else if (orderingMode === 'oldest') {
        orderedActivities = [...published].sort((a, b) => (a.date || '').localeCompare(b.date || ''));
      } else {
        // Default: latest first
        orderedActivities = [...published].sort((a, b) => (b.date || '').localeCompare(a.date || ''));
      }

      const previewLimit = currentConfig.homeActivitiesLimit || 6;
      setActivities(orderedActivities.slice(0, previewLimit));

      setNotices(storageService.getNotices().filter((n) => n.isPublished).slice(0, 3));
      // Load all published active members sorted by serial; exact display count will be sliced before rendering
      setMembers(
        storageService
          .getMembers()
          .filter((m) => m.isActive)
  ...
  return (
    <div className="space-y-16 sm:space-y-20 pb-16">
      ...
      <section className="relative overflow-hidden bg-gradient-to-b from-[#F7F5F0] via-[#FDFCF9] to-[#FDFCF9] pt-12 pb-16 sm:pt-20 sm:pb-24 border-b border-[#EBE8E0]">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          {(config.locationDisplayMode !== 'hidden' || (config.showFoundingDate !== false && Boolean(config.foundedDate))) && (
            <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-4 text-xs sm:text-sm text-[#7A877E] mt-5 max-w-2xl mx-auto px-2">
              {config.locationDisplayMode !== 'hidden' && (
                <FoundationLocationDisplay config={config} variant="inline" />
              )}
              {config.locationDisplayMode !== 'hidden' && config.showFoundingDate !== false && Boolean(config.foundedDate) && (
                <span className="text-[#C5BFB0] hidden sm:inline">•</span>
              )}
      ...
      {/* 4. SELECTED ACTIVITIES PREVIEW (Horizontal Rail Layout) */}
      {config.showHomeActivities !== false && (() => {
        const isMobile = windowWidth < 640;
        const isTablet = windowWidth >= 640 && windowWidth < 1024;
        const isWideDesktop = windowWidth >= 1280;

        const activityCardsPerView = isMobile
          ? (config.homeActivitiesMobileCols || 1)
          : isTablet
          ? (config.homeActivitiesTabletCols || 2)
          : isWideDesktop
        ...
        const activityCardStyle: React.CSSProperties = {
          ...
          maxWidth: activityFlexBasis,
          scrollSnapAlign: 'start',
        };

        const scrollActivitiesRail = (direction: 'prev' | 'next') => {
          if (!activitiesRailRef.current) return;
          const rail = activitiesRailRef.current;
          const cardEl = rail.querySelector('article') as HTMLElement;
          const step = cardEl ? cardEl.offsetWidth + activityGapPx : 300;
          rail.scrollBy({
        ...
        const eyebrowText = tMulti(config.homeActivitiesEyebrow);
        const titleText = tMulti(config.homeActivitiesTitle) || t('sectionActivitiesTitle');
        const subtitleText = tMulti(config.homeActivitiesSubtitle);
        const viewAllLabel =
          tMulti(config.homeViewAllActivitiesLabel) ||
          (language === 'bn' ? 'সব কার্যক্রম দেখুন' : language === 'ar' ? 'عرض جميع الأنشطة' : 'View All Activities');

        return (
          <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            {/* Header: Title & Desktop Prev/Next Controls */}
            <div className="flex items-end justify-between gap-4">
              <div>
                {eyebrowText && (
                  <span className="inline-flex items-center px-2.5 py-0.5 rounded-full bg-[#E8EFEA] text-[#2D5A41] text-[11px] font-bold mb-1.5">
                    {eyebrowText}
                  </span>
                )}
                ...
                {subtitleText && (
                  <p className="text-xs sm:text-sm text-[#5C665F] mt-1 max-w-2xl leading-relaxed">
                    {subtitleText}
                  </p>
                )}
              </div>

              {/* Prev / Next controls for horizontal rail */}
              {activities.length > activityCardsPerView && (
                <div className="hidden sm:flex items-center gap-2 shrink-0 pb-1">
                  <button
                    type="button"
                    onClick={() => scrollActivitiesRail('prev')}
                    className="p-2 rounded-xl border border-[#EBE8E0] bg-white text-[#5C665F] hover:text-[#2D5A41] hover:border-[#2D5A41] hover:bg-[#F7F5F0] transition-all shadow-3xs cursor-pointer focus:outline-hidden focus:ring-2 focus:ring-[#2D5A41]"
                    title="আগের কার্যক্রম"
                  ...
                  <button
                    type="button"
                    onClick={() => scrollActivitiesRail('next')}
                    className="p-2 rounded-xl border border-[#EBE8E0] bg-white text-[#5C665F] hover:text-[#2D5A41] hover:border-[#2D5A41] hover:bg-[#F7F5F0] transition-all shadow-3xs cursor-pointer focus:outline-hidden focus:ring-2 focus:ring-[#2D5A41]"
                    title="পরবর্তী কার্যক্রম"
            ...
            {activities.length > 0 ? (
              ...
              <div className="p-8 bg-white rounded-2xl border border-[#EBE8E0] text-center text-xs sm:text-sm text-[#7A877E]">
                {tMulti(config.homeActivitiesEmptyText) || 'শিগগিরই আমাদের কার্যক্রমের নতুন তথ্য এখানে প্রকাশিত হবে।'}
              </div>
            )}

            {/* Bottom "সব কার্যক্রম দেখুন" Button */}
            {config.homeShowViewAllActivities !== false && (
              <div className="flex justify-center pt-2">
                <Link
                  to="/activities"
                  className="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl bg-white border border-[#EBE8E0] hover:border-[#2D5A41] text-[#2D5A41] hover:text-[#234733] text-xs sm:text-sm font-bold shadow-3xs hover:shadow-xs transition-all duration-200 group focus:outline-hidden focus:ring-2 focus:ring-[#2D5A41]"
                >
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {members.length > 0 ? (
            (() => {
              ...
              const desktopCols = config.homeMemberGrid?.desktopCols || 4;
              const desktopRows = config.homeMemberGrid?.desktopRows || 2;
              const desktopLimit =
                config.homeMemberGrid?.desktopLimit ||
                config.homeMembersCount ||
                (desktopCols * desktopRows);

              const mobileCols = config.homeMemberGrid?.mobileCols || 2;
              const mobileRows = config.homeMemberGrid?.mobileRows || 2;
              const mobileLimit =
                config.homeMemberGrid?.mobileLimit ||
                (mobileCols * mobileRows);

              // Tablet sensible responsive fallback
              const tabletCols = Math.min(desktopCols, 3);
              const tabletLimit = Math.min(desktopLimit, tabletCols * (desktopRows || 2));

              // Determine exact limit based on device/viewport
              const effectiveLimit = isMobile ? mobileLimit : isTablet ? tabletLimit : desktopLimit;

              // SLICE ARRAY BEFORE MAPPING - strict requirement
              const displayMembers = members.slice(0, effectiveLimit);
              ...
              const tabletClass =
                tabletCols === 2 ? 'sm:grid-cols-2' : 'sm:grid-cols-3';
              ...
              return (
                <div>
                  <div className={`grid ${mobileClass} ${tabletClass} ${desktopClass} gap-3 sm:gap-4`}>
                    {displayMembers.map((member) => {
                      ...
                      return (
                        <div
                          key={member.id}
                          className="p-3.5 sm:p-4 rounded-xl bg-white border border-[#EBE8E0] hover:border-[#2D5A41]/40 shadow-2xs hover:shadow-xs transition-all text-center group flex flex-col items-center relative overflow-hidden"
                        >
                          <Link
                            to={`/members/${member.id}`}
                            className="flex flex-col items-center w-full focus:outline-hidden"
                          >
                            {/* Member Serial Corner Badge */}
                            <MemberSerialBadge
                          ...
                          {/* Member Social Links - separate from Link, completely avoiding nested <a> */}
                          <div className="mt-1 relative z-10 w-full flex justify-center">
                            <MemberSocialIcons
                              member={member}
                              size="sm"
                            />
                  ...
                  {/* Polished "সব সদস্য দেখুন" button below preview grid */}
                  {config.homeShowViewAllMembers !== false && (
                    <div className="mt-8 flex justify-center">
                      <Link
                        to="/members"
                        className="inline-flex items-center gap-2.5 px-6 py-3 rounded-xl bg-white hover:bg-[#2D5A41] text-[#2D5A41] hover:text-white border-2 border-[#2D5A41]/25 hover:border-[#2D5A41] font-bold text-xs sm:text-sm shadow-2xs hover:shadow-md transition-all duration-200 group"
                      >
                        <span>
                          {config.homeViewAllMembersLabel
                            ? tMulti(config.homeViewAllMembersLabel)
                            : (language === 'bn' ? 'সব সদস্য দেখুন' : t('viewAll'))}
                        </span>
