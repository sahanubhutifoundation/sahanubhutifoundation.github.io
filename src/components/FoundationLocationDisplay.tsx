    ...
    if (hasValidMapUrl) {
      return (
        <a
          ...
          rel="noopener noreferrer"
          title={mapTooltip}
          className={`flex items-start sm:items-center gap-2 p-2.5 bg-white hover:bg-[#FDFCF9] rounded-lg border border-[#EBE8E0] hover:border-[#2D5A41]/40 text-[#5C665F] hover:text-[#2D3630] transition-all group ${className}`}
        >
          {showIcon && (
            <MapPin className="w-4 h-4 text-[#2D5A41] shrink-0 mt-0.5 sm:mt-0 group-hover:scale-105 transition-transform" />
          )}
          <span className="text-xs sm:text-sm break-words leading-snug group-hover:underline underline-offset-2 flex-1 text-left">
            {locationText}
          </span>
          <ExternalLink className="w-3 h-3 text-[#7A877E] shrink-0 opacity-70 mt-0.5 sm:mt-0" />
        </a>
      );
    }
    return (
      <div
        className={`flex items-start sm:items-center gap-2 p-2.5 bg-white rounded-lg border border-[#EBE8E0] text-[#5C665F] ${className}`}
      >
        {showIcon && <MapPin className="w-4 h-4 text-[#2D5A41] shrink-0 mt-0.5 sm:mt-0" />}
        <span className="text-xs sm:text-sm break-words leading-snug text-left">{locationText}</span>
      </div>
    );
  }
  ...
  if (hasValidMapUrl) {
    return (
      <a
        ...
        rel="noopener noreferrer"
        title={mapTooltip}
        className={`inline-flex items-start sm:items-center gap-1.5 hover:text-[#2D5A41] transition-colors group cursor-pointer text-left ${className}`}
      >
        {showIcon && (
          <MapPin className="w-4 h-4 text-[#2D5A41] group-hover:scale-110 transition-transform shrink-0 mt-0.5 sm:mt-0" />
        )}
        <span className="group-hover:underline underline-offset-2 break-words">{locationText}</span>
        <ExternalLink className="w-3 h-3 opacity-60 group-hover:opacity-100 transition-opacity shrink-0 mt-0.5 sm:mt-0" />
      </a>
    );
  }

  return (
    <span className={`inline-flex items-start sm:items-center gap-1.5 text-left ${className}`}>
      {showIcon && <MapPin className="w-4 h-4 text-[#2D5A41] shrink-0 mt-0.5 sm:mt-0" />}
